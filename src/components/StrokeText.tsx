...
export type StrokeTextProps = {
  ...
  strokeColor?: string;
  fillColor?: string;
  /** Optional gradient stops for the fill (left → right). Falls back to `fillColor`. */
  fillGradient?: string[];
  strokeWidth?: number;
  drawDuration?: number;
  fillDelay?: number;
...
export default function StrokeText({
  ...
  const fillGradientId = `stroke-text-fill-${rawId.replace(/[^a-zA-Z0-9_-]/g, "")}`;

  const characters = useMemo(() => Array.from(String(text ?? "")), [text]);
  const hasFillGradient = Boolean(fillGradient && fillGradient.length > 1);
  const resolvedFill = hasFillGradient ? `url(#${fillGradientId})` : fillColor;
  ...
  const fontStyle = useMemo(
    ...
    [numericFontSize, fontWeight, letterSpacing]
  );
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, numericFontSize * 0.03);
      // Tight ink box — avoids empty “air” above/below glyphs inside the SVG
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  return (
    <span
      ...
      style={
        {
          "--stroke-text-height": `${Math.round(numericFontSize * 1.15)}px`,
          ...style,
        } as CSSProperties
      }
      role="img"
      ...
      >
        <defs>
          {hasFillGradient ? (
            ...
            >
              {fillGradient!.map((stop, i) => (
                <stop
                  key={`${stop}-${i}`}
                  offset={`${(i / (fillGradient!.length - 1)) * 100}%`}
                  stopColor={stop}
                />
              ))}
            </linearGradient>
